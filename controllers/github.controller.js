...
module.exports = {
  ...
  createRepo: async (req, res) => {
    ...
    const userId =
      Env == "test"
        ? req.headers.id
        : await authenticationManager.getAuthenticatedUserId(req, res);
    ...
    try {
      ...
      const newRepo = await repoManager.saveNewRepoToDataBase(
        repoName,
        user.username,
        user.id,
        [],
        [],
        proxyUrl,
        repoCreationResp.data.html_url
      );
      ...
      //--------------------------------------------------------------------------------- //
      await userManager.addRepoToUserProfile(
        newRepo.id,
        newRepo.repo_name,
        user.username,
        proxyUrl,
  ...
  deleteRepo: async (req, res) => {
    ...
    if (await repoManager.userIsAdminOfRepo(userId, req.params.repoName)) {
      ...
      const removeRepoFromDb = await repoManager.deleteRepoFromDataBase(
        repo.id
      );

      // TODO:Add a way to delete the cards
      const removeRepoFromUserProfile = await userManager.removeRepoFromUserProfile(
        repo.id,
        repo.repo_name,
        repo.repo_creator_name
      );
  ...
  createBranch: async (req, res) => {
    ...
    if (
      !requestBody.taskId ||
      !requestBody.parentRefHash ||
      !req.params.repoName ||
      !requestBody.userInfo.userName
    ) {
      return res
        .status(400)
        .send("Cant complete this request based on information given");
    }
    if (
      (await repoManager.userIsAdminOfRepo(userId, req.params.repoName)) ||
      (await repoManager.userIsCollaboratorOfRepo(userId, req.params.repoName))
    ) {
      const authToken =
        Env === "test" ? req.headers.user : req.user.accessToken;
      ...
      const addBranchToTaskRes = await taskManager.addGitBranchToTask(
        { refName: refInfo.ref, gitInfo: refInfo.object },
        req.body.parentRefHash,
        parentBranchName,
        taskId
      );
  ...
  deleteBranch: async (req, res) => {
    ...
    if (
      (await repoManager.userIsAdminOfRepo(userId, req.body.repo)) ||
      ...
      try {
        ...
        try {
          const removeBranchFromTaskRes = await taskManager.removeGitBranchFromTask(
            gitRef,
            taskId
          );
  ...
  createPullRequest: async (req, res) => {
    ...
    try {
      ...
      if (pullRequestCreationResponse.status === 201) {
        ...
        const addPullRequestToTaskRes = await taskManager.addPullRequestToTask(
          title,
          pullRequestUrl,
          taskId
        );
  ...
  updateTaskStatus: async (req, res) => {
    ...
    const requestBody = req.body;
    const repoName = req.params.repoName;
    const taskUpdateResp = await repoManager.updateTaskStatusWithinRepo(
      requestBody.taskId,
      repoName,
      requestBody.taskState
    );
    const updateTaskCollection = await taskManager.updateTaskStatus(
      requestBody.taskId,
      requestBody.taskState
    );
