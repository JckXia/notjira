...
const data = [{ cardId: 1, cardState: "InProg", toDoItem: "Testing for crx" }];
...
class RepoWorkSpace extends Component {
  ...
  onDragEnd = result => {
    ...
    try {
      GraphQLHelperFunction.updateTask(
        data.taskId,
        currentRepo,
        newTaskStatus
      ).then(res => {});
  ...
  render() {
    ...
    if (this.state.empty == true) {
      return (
        <div>
          <h3>
            <code>
              Looks like you have an empty repo.
              <br />
            </code>
            <h5>
              <code>
                Please follow instruction on github to set up this repo
              </code>
            </h5>
          </h3>
        </div>
    ...
    if (auth == true) {
            ...
            <Container>
              {this.state.columnOrder.map(columnId => {
                ...
                return (
                  <Column
                    {...this.props}
                    repoBranches={this.state.branch}
                    key={column.id}
      ...
      return (
        <h3>
          {" "}
          <code>Forbidden 403. Please login!</code>
        </h3>
      );
    }
  }
